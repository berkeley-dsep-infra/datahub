...
def git_clone():
    if os.path.exists(os.path.join(local_repo, repo_dirname)):
        return
    out = subprocess.check_output(["git", "clone", args.repo], cwd=local_repo).decode(
        "utf-8"
    )
    return out
...
parser.add_argument(
    "-f", dest="filename", required=True, help="File containing user emails"
)
parser.add_argument("-r", dest="repo", default=repo, help="Course asset repo")
